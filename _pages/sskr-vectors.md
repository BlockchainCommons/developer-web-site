...
As explained in the [Uniform Resource (UR)](/ur/) specification, URs are built from a minimal (two-character) version of [ByteWords](/bytewords/). Where URs self identify with their prefix (`ur:sskr`), Bytewords instead self identify with a ByteWords prefix (`tuna next keep`).
...
```
***Breaking Down the Test Vector:*** Converting the minimal [ByteWords](/bytewords/) of the [UR](/ur/) to hex and decoding it at [cbor.me](https://cbor.me/) reveals the following structure:
...
```
As explained in the [Uniform Resource (UR)](/ur/) specification, URs are built from a minimal (two-character) version of [ByteWords](/byetwords/). Where URs self identify with their prefix (`ur:sskr`), Bytewords instead self identify with a ByteWords prefix (`tuna next keep`):
```
...
The UR then uses the minimal [ByteWords](/bytewords/) form, meaning that it
shows only the first and last letter of each word. See the [256-bit 2-of-3 vector](https://developer.blockchaincommons.com/sskr/vectors/#two-of-three-sskr-1) for a full explanation of how ByteWords and UR formats interrelate

## Final Notes

SSKR is a powerful way to back up digital secrets. This example
demonstrates two specific seeds and how they can be used to generate SSKR
shares using two different groupings, either in the current SSKR Envelope format or the legacy SSKR UR format.
